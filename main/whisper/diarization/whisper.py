"""
Whisper-based speech recognition and transcription for speaker diarization.

This module provides Whisper model loading, transcription, and word-level
timestamp alignment capabilities.
"""
...
def load_model(name: str = "base", device: str = "cpu"):
    """
    Load a Whisper model from checkpoint.

    Args:
        name: Model name (e.g., 'base', 'small', 'medium')
        device: Device to load model on ('cpu' or 'cuda')

    Returns:
        Loaded Whisper model
    """
    checkpoint_file = os.path.join("models", name + ".pt")
    ...
    try:
        ...
        logger.info(f"Loaded Whisper model '{name}' to {device}")
...
def dtw_cpu(x):
    ...
    cost = np.ones((N + 1, M + 1), dtype="float32") * np.inf
    trace = -np.ones((N + 1, M + 1), dtype="float32")
    cost[0, 0] = 0
...
def log_mel_spectrogram(audio, n_mels = 80, padding = 0, device = None):
    ...
    if not torch.is_tensor(audio):
        if isinstance(audio, str):
            ...
            audio = load_audio(audio, sample_rate=SAMPLE_RATE).astype("float32")
    ...
    if str(audio.device).startswith(("privateuseone")):
        if stft is None: 
...
def transcribe_function(
    ...
    **decode_options
    ...
    with tqdm.tqdm(total=content_frames, unit="frames", ncols=80, colour='BLUE', disable=verbose is not False, mininterval=0.5) as pbar:
