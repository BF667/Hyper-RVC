"""
Segment and Timeline classes for speaker diarization.

Provides data structures for managing audio segments and timelines,
including operations like union, intersection, cropping, and gap detection.
"""

import numpy as np

from sortedcontainers import SortedList
from typing import Optional, Iterator, Tuple, List, Union, Any


class Timeline:
    """
    A collection of non-overlapping segments representing audio timelines.
    
    Supports operations like union, intersection, cropping, and gap detection.
    """
    @classmethod
    def from_df(cls, df, uri: str = None):
        """Create a Timeline from a pandas DataFrame."""
    ...
    def __init__(
        ...
        Args:
            ...
            uri: Optional URI identifier for the timeline
        """
        if segments is None:
            segments = ()
...
class Segment:
    ...
    Supports operations like intersection, union, and duration calculation.
    """
    ...
    @staticmethod
    def set_precision(ndigits: int = None):
        """
        Set the precision for segment time comparisons.
        
        Args:
            ndigits: Number of decimal places, or None to disable rounding
        """
    ...
    def __bool__(self):
        """Return True if segment has positive duration."""
        return bool((self.end - self.start) > SEGMENT_PRECISION)

    def __post_init__(self):
        """Round times if AUTO_ROUND_TIME is enabled."""
        if AUTO_ROUND_TIME:
...
class SlidingWindow:
    ...
    def crop(self, focus, mode = 'loose', fixed = None, return_ranges = False):
        ...
        return np.array(range(*rng), dtype="int64")
