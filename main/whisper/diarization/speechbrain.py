"""
SpeechBrain integration for speaker diarization.

Provides pretrained model loading, distributed training utilities,
...
class MainProcessContext:
    """Context manager for main process only operations."""
    def __enter__(self):
...
def main_process_only(function: Callable) -> Callable:
    """
    Decorator to run function only on main process in distributed training.
...
class Pretrained(torch.nn.Module):
    """
    Base class for pretrained SpeechBrain models.
    
    Handles module initialization, distributed training setup,
    and model compilation.
    """
    HPARAMS_NEEDED: List[str] = []
    ...
    def __init__(
        ...
        Args:
            modules: Dictionary of model modules
...
class EncoderClassifier(Pretrained):
    """
    Encoder-based classifier for speaker recognition.
    
    Provides methods for encoding audio batches and classification.
    """
    MODULES_NEEDED = [
        "compute_features",
        "mean_var_norm",
    ...
    def encode_batch(self, wavs, wav_lens=None, normalize=False):
        ...
        Args:
            wavs: Waveform tensor of shape (batch, time)
            wav_lens: Relative lengths of each waveform
    ...
    def classify_batch(self, wavs, wav_lens=None):
        ...
        Args:
            wavs: Waveform tensor
            wav_lens: Relative lengths
            
        Returns:
            Tuple of (probabilities, scores, indices, labels)
        """
