...
def run_file(model, args, config, device, verbose=False):
    ...
    # Add progress bar for audio loading
    with tqdm(total=1, desc="Loading audio", unit="file", ncols=80, colour='GREEN', leave=False) as pbar:
        try:
    ...
    # Add progress bar for TTA processing
    full_result = []
    tta_desc = "Processing with TTA" if args.use_tta else "Processing"
    with tqdm(total=len(track_proc_list), desc=tta_desc, unit="variant", ncols=80, colour='CYAN', leave=False) as pbar:
    ...
    with tqdm(total=1, desc="Averaging results", unit="step", ncols=80, colour='YELLOW', leave=False) as pbar:
    ...
    with tqdm(total=len(instruments), desc="Saving files", unit="stem", ncols=80, colour='MAGENTA', leave=False) as pbar:
...
def proc_file(args):
    ...
    parser.add_argument(
        "--fp16",
        action="store_true",
        help="Use FP16 precision for faster inference (if supported by GPU).",
    )
    if args is None:
    ...
    with tqdm(total=1, desc="Loading model", unit="model", ncols=80, colour='BLUE', leave=False) as pbar:
