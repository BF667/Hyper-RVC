"""
F0 Pitch Generator for Hyper-RVC.

Ported from Vietnamese-RVC (https://github.com/PhamHuynhAnh16/Vietnamese-RVC).
Supports 15+ F0 methods including hybrid, autotune, proposal pitch, and realtime
inference.  Every predictor is lazily loaded on first use.

Supported methods
-----------------
rmvpe, hpa-rmvpe, crepe, crepe-tiny, mangio-crepe, mangio-crepe-tiny,
fcpe, fcpe-legacy, fcpe-previous, onnxcrepe, harvest, dio, stonemask,
pm (ac/cc/shs), yin, pyin, piptrack, swipe, stonemask-swipe,
penn, mangio-penn, djcm, djcm-svs, djcm-clipping, pesto, swift,
hybrid[*+*]
"""
...
def post_process(
    ...
    """Convert raw F0 to mel-spectral bin indices for the RVC pipeline."""
...
def realtime_post_process(
    ...
    """Realtime variant of post_process that operates on torch tensors."""
    f0 *= 2 ** (f0_up_key / 12)
...
class Generator:
    """Comprehensive F0 pitch generator with 15+ methods.

    Replaces the earlier ``F0Generator`` class.  Supports autotune,
    proposal pitch, manual F0 override, realtime inference, and
    weighted-geometric-mean hybrid blending.

    Args:
        sample_rate: Audio sample rate for predictors (default 16000).
        hop_length: Hop length in samples (default 160).
        f0_min: Minimum detectable F0 in Hz.
        f0_max: Maximum detectable F0 in Hz.
        alpha: Blend weight for hybrid methods (0.5 = equal).
        is_half: Whether to use FP16 for models that support it.
        device: torch device string (e.g. ``"cuda:0"``, ``"cpu"``).
        predictor_onnx: If *True*, predictors that support ONNX will
            load ``.onnx`` files instead of ``.pt``.
        delete_predictor_onnx: If *True* AND *predictor_onnx* is enabled,
            ONNX predictors are deleted after each call to free VRAM.
        providers: List of ONNX execution provider names.
    """
    ...
    def __init__(
        ...
        self.ref_freqs = [
            ...
            77.78, 82.41, 87.31, 92.50, 98.00, 103.83, 110.00, 116.54,
            123.47, 130.81, 138.59, 146.83, 155.56, 164.81, 174.61,
            185.00, 196.00, 207.65, 220.00, 233.08, 246.94, 261.63,
            277.18, 293.66, 311.13, 329.63, 349.23, 369.99, 392.00,
            415.30, 440.00, 466.16, 493.88, 523.25, 554.37, 587.33,
            622.25, 659.25, 698.46, 739.99, 783.99, 830.61, 880.00,
    ...
    def calculator(
        ...
        """Full inference pipeline: F0 estimation -> autotune -> post-process.

        Returns:
            Tuple of ``(f0_mel, f0)`` where *f0_mel* is an int32 array of
            mel bin indices and *f0* is the raw F0 array in Hz.
        """
        if p_len is None:
        ...
        if proposal_pitch:
            up_key = proposal_f0_up_key(f0, proposal_pitch_threshold, 12)
            logger.debug(f"Proposal F0 up-key: {up_key}")
            f0_up_key += up_key

        if f0_autotune:
            logger.debug("Starting autotune...")
    ...
    def realtime_calculator(
        ...
        """Realtime variant of ``calculator`` operating on torch tensors."""
    ...
    def _resize_f0(self, x, target_len):
        """Resize F0 array to *target_len* using linear interpolation."""
        if len(x) == target_len:
            return x
    ...
    def _model_path(self, filename: str) -> str:
        """Join the predictors base directory with a model file name."""
        return os.path.join(get_predictors_dir(), filename)
    ...
    def compute_f0(self, f0_method, x, p_len, filter_radius):
        """Dispatch to the appropriate ``get_f0_*`` method by name."""
        if "pm" in f0_method:
            f0 = self.get_f0_pm(
                x, p_len, filter_radius=filter_radius, mode=f0_method.split("-")[1]
            )
            ...
            f0 = (
                self.get_f0_mangio_crepe(x, p_len, split_f0[2])
                if split_f0[0] == "mangio"
                else self.get_f0_crepe(x, p_len, split_f0[1], filter_radius=filter_radius)
            )
            ...
            f0 = self.get_f0_librosa(
                x, p_len, mode=f0_method, filter_radius=filter_radius
            )
            ...
            f0 = (
                self.get_f0_mangio_penn(x, p_len)
                if f0_method.split("-")[0] == "mangio"
                else self.get_f0_penn(x, p_len, filter_radius=filter_radius)
            )
            ...
            raise ValueError(f"F0 method not recognised: '{f0_method}'")

        if isinstance(f0, tuple):
            f0 = f0[0]
        if "medfilt" in f0_method or "svs" in f0_method:
            f0 = medfilt(f0, kernel_size=5)

        return f0

    # ------------------------------------------------------------------
    # Hybrid F0 (weighted geometric mean)
    # ------------------------------------------------------------------

    def get_f0_hybrid(self, methods_str, x, p_len, filter_radius):
        """Estimate F0 using a weighted geometric mean of multiple methods."""
        methods_str = re.search(r"hybrid\[(.+)\]", methods_str)
        if methods_str:
            methods = [m.strip() for m in methods_str.group(1).split("+")]
        ...
        f0_mix[voiced_mask] = np.exp(
            np.nansum(np.log(stacked + 1e-6) * weights[:, None], axis=0)[
                voiced_mask
            ]
        )
    ...
    # ------------------------------------------------------------------
    # Individual F0 methods (lazy-loaded)
    # ------------------------------------------------------------------
    ...
    def get_f0_mangio_crepe(self, x, p_len, model="full"):
        """F0 via CREPE with mangio preprocessing (normalisation + Viterbi)."""
    ...
    def get_f0_fcpe(self, x, p_len, legacy=False, previous=False, filter_radius=3):
        """F0 via FCPE (supports legacy and previous model variants)."""
            ...
            self.fcpe = FCPE(
                ...
                self._model_path(
                    (
                        "fcpe_legacy"
                        if legacy
                        else ("fcpe" if previous else "ddsp_200k")
                    )
    ...
    def get_f0_rmvpe(self, x, p_len, clipping=False, filter_radius=3, hpa=False, previous=False):
        """F0 via RMVPE (with optional HPA variant and clipping)."""
        ...
        f0 = (
            self.rmvpe.infer_from_audio_with_pitch(
                x, thred=filter_radius, f0_min=self.f0_min, f0_max=self.f0_max
            )
            if clipping
            else self.rmvpe.infer_from_audio(x, thred=filter_radius)
        )
    ...
    def get_f0_pyworld(self, x, p_len, filter_radius, model="harvest", use_stonemask=True):
        """F0 via pyworld (harvest or dio + optional stonemask)."""
        if not hasattr(self, "pw"):
            from main.library.predictors.WORLD.WORLD import PYWORLD

            self.pw = PYWORLD(
                self._model_path("world"),
                self._model_path("world.bin"),
            )
        ...
        if use_stonemask:
            f0 = stonemask(x, self.sample_rate, t, f0)
    ...
    def get_f0_swipe(self, x, p_len, filter_radius=3, use_stonemask=True):
        """F0 via SWIPE (with optional stonemask refinement)."""
        f0, t = swipe(
            x.astype(np.float32),
            self.sample_rate,
            f0_floor=self.f0_min,
    ...
    def get_f0_librosa(self, x, p_len, mode="yin", filter_radius=3):
        """F0 via librosa (yin, pyin, or piptrack)."""
        if mode != "piptrack":
    ...
    def get_f0_penn(self, x, p_len, filter_radius=3):
        """F0 via PENN with mean/median post-filtering."""
        if not hasattr(self, "penn"):
            from main.library.predictors.PENN.PENN import PENN

            self.penn = PENN(
                self._model_path(f"fcn.{'onnx' if self.predictor_onnx else 'pt'}"),
    ...
    def get_f0_mangio_penn(self, x, p_len):
        """F0 via PENN with mangio preprocessing (normalisation)."""
    ...
    def get_f0_djcm(self, x, p_len, clipping=False, svs=False, filter_radius=3):
        ...
        f0 = (
            ...
            if clipping
            else self.djcm.infer_from_audio(x, thred=filter_radius)
        )
    ...
    def get_f0_pesto(self, x, p_len):
        """F0 via PESTO (neural network with harmonic CQT preprocessing)."""
        if not hasattr(self, "pesto"):
            from main.library.predictors.PESTO.PESTO import PESTO

            self.pesto = PESTO(
                self._model_path(f"pesto.{'onnx' if self.predictor_onnx else 'pt'}"),
        ...
        if self.predictor_onnx and self.delete_predictor_onnx:
            del self.pesto.model, self.pesto
    ...
    def get_f0(
        ...
        """Backward-compatible dispatch (old F0Generator API).

        Maps to ``compute_f0`` for recognised methods.  Falls back to
        the legacy dispatch table for ``onnxcrepe`` and ``harvest``
        (which are Hyper-RVC-specific predictors not in the Vietnamese-RVC
        Generator).
        """
        # Vietnamese-RVC methods
        vn_methods = {
            "rmvpe": "rmvpe",
            "crepe": "crepe-full",
            ...
            "fcpe": "fcpe",
            "harvest": "harvest",
            "onnxcrepe": None,  # handled below
            "hpa-rmvpe": "hpa-rmvpe",
        }

        if f0_method == "onnxcrepe":
            # Hyper-RVC-specific ONNX CREPE predictor
            if not hasattr(self, "onnxcrepe"):
                from main.library.predictors.ONNXCREPE.ONNXCREPE import ONNXCrepe
                from main.tools.variables import get_onnxcrepe_model_path

                self.onnxcrepe = ONNXCrepe(
                    get_onnxcrepe_model_path("full"),
                    f0_min=self.f0_min,
                    f0_max=self.f0_max,
                    device=self.device,
                    sample_rate=self.sample_rate,
            ...
            thred = filter_radius / 100.0
            f0 = (
                self.onnxcrepe.infer_from_audio_with_pitch(
                    x, thred=thred, f0_min=self.f0_min, f0_max=self.f0_max
                )
                if clipping
                else self.onnxcrepe.infer_from_audio(x, thred=thred)
            )
            if self.delete_predictor_onnx:
                del self.onnxcrepe
            return self._resize_f0(f0, p_len)

        if f0_method == "harvest":
            # Use pyworld harvest directly (Hyper-RVC HARVEST predictor)
            if not hasattr(self, "harvest predictor"):
                from main.library.predictors.HARVEST.HARVEST import HARVEST

                self.harvest_predictor = HARVEST(
                    f0_min=self.f0_min,
                    f0_max=self.f0_max,
                    sample_rate=16000,
                )
            ...
            f0 = (
                self.harvest_predictor.infer_from_audio_with_pitch(
                    x, f0_min=self.f0_min, f0_max=self.f0_max
                )
                if clipping
                else self.harvest_predictor.infer_from_audio(x)
            )
            return self._resize_f0(f0, p_len)

        # Map to Vietnamese-RVC method names
        vn_method = vn_methods.get(f0_method, f0_method)

        fr = filter_radius if filter_radius % 2 != 0 else filter_radius + 1

        if "clipping" in vn_method or clipping:
            vn_method = vn_method + "-clipping"

        f0 = self.compute_f0(vn_method, x, p_len, fr)
    ...
    def get_f0_hybrid_compat(self, methods_str, x, p_len, hop_length=160):
        """Backward-compatible hybrid F0 with old signature."""
        return self.get_f0_hybrid(methods_str, x, p_len, hop_length)
    ...
    def clear_cache(self):
        """Remove all cached predictor instances to free memory."""
        for attr in (
            "rmvpe", "crepe", "crepe_tiny", "fcpe",
            "onnxcrepe", "harvest", "mangio_crepe", "penn",
            "mangio_penn", "djcm", "swift", "pesto", "pw",
        ):
