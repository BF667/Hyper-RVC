...
class Pipeline:
    ...
    def __init__(self, tgt_sr, config):
        ...
        # F0 generator — lazy-loads predictors on first use
        self.f0_gen = F0Generator(
            device=self.device,
    ...
    def get_f0(
        ...
        Uses F0Generator for lazy-loaded predictor dispatch.
        ...
        Args:
            ...
            f0_method: Method to use for F0 estimation (e.g., "rmvpe").
            filter_radius: Radius for voicing threshold (0–100).
            hop_length: Hop length for F0 estimation methods.
            f0_autotune: Whether to apply autotune to the F0 contour.
            inp_f0: Optional input F0 contour to use instead of estimating.
        """
        if "hybrid" in f0_method:
            f0 = self.f0_gen.get_f0_hybrid(f0_method, x, p_len, hop_length)
        else:
            f0 = self.f0_gen.get_f0(
                x,
                p_len=p_len,
                f0_method=f0_method,
        ...
        f0_coarse = np.rint(f0_mel).astype("int64")
